...
for (i in 1:nrow(meta)) {
  ...
  #casper---------------------------------------------------------------------
    ...
    obj <- readRDS(obj_fn)
    expr <- obj@control.normalized.noiseRemoved[[3]]  # try running without log2()
  ...
  #infercnv------------------------------------------------------------------
    ...
    c0 <- hclust(dist(t(expr)))
    saveRDS(c0, sprintf("%s.%s_hclust.cluster.rds", sid, out_id))
    c_n <- obj@tumor_subclusters$hc$Normal     # update the cluster names!
    c_t <- obj@tumor_subclusters$hc$Malignant
    for (k in all_k) {
      c1 <- cutree(tree = c0, k = k)
      saveRDS(c1, sprintf("k%d/%s.%s_hclust.cluster.k%d.rds", k, sid, out_id, k))
