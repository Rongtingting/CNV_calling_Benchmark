# BCH869.run.fast2.R - benchmark on BCH869 dataset
...
roc_fn <- "BCH869.copy_gain.gene_scale.roc.pre_plot_dat_list.list.rds"
prc_fn <- "BCH869.copy_gain.gene_scale.prc.pre_plot_dat_list.list.rds"
metrics <- c("ROC", "PRC")
metric_fn <- c(roc_fn, prc_fn)

gene_anno_fn <- "annotate_genes_hg19_update.txt"
cell_subset_fn <- "BCH869.copy_gain.gene_scale.subset.cells.df.tsv"
gene_subset_fn <- "BCH869.copy_gain.gene_scale.subset.genes.df.tsv"
truth_fn <- "BCH869.copy_gain.gene_scale.truth.cell_x_gene.binary.mtx.rds"
out_dir <- "result"
