# simu_eval.py - generate running scripts for performance evaluation.
...
''' % (conf.sid, cnv_type, cnv_scale)
    ...
    if cnv_type in ("copy_gain", "copy_loss"):
        s += '''
method_list <- c("numbat", "xclone")
method_sub_list <- c("numbat", "xclone")
mtx_type_list <- c("prob", "prob")
dat_dir_list <- c(
  "%s",
  "%s"
)
